...
internal class GradleClassPathResolver(
    ...
    companion object {
        ...
        fun maybeCreate(file: Path): GradleClassPathResolver? =
            file
                .takeIf { file.endsWith("build.gradle") || file.endsWith("build.gradle.kts") }
                ?.let {
                    GradleClassPathResolver(it, includeKotlinDSL = file.toString().endsWith(".kts"))
                }
    }
}
...
private fun readDependenciesViaGradleCLI(
    ...
    val command =
        "$gradle ${tmpScripts.map { "-I $it" }.joinToString(" ")} ${gradleTasks.joinToString(" ")} --console=plain"
    val dependencies =
