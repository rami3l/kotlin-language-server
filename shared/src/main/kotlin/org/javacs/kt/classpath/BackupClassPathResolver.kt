...
object BackupClassPathResolver : ClassPathResolver {
    ...
    override val classpath: Set<ClassPathEntry>
        get() =
            findKotlinStdlib()?.let { setOf(it) }.orEmpty().map { ClassPathEntry(it, null) }.toSet()
}
...
private fun tryFindingLocalArtifactUsing(
    ...
    val isCorrectArtifact =
        BiPredicate<Path, BasicFileAttributes> { file, _ ->
...
private fun Path.existsOrNull() = if (Files.exists(this)) this else null
...
private fun findLocalArtifactDirUsingGradle(group: String, artifact: String) =
    LocalArtifactDirectoryResolution(
        gradleCaches?.resolve(group)?.resolve(artifact)?.existsOrNull(),
        "Gradle"
    )

// TODO: Resolve the gradleCaches dynamically instead of hardcoding this path
private val gradleCaches by lazy {
    gradleHome.resolve("caches").resolveStartingWith("modules").resolveStartingWith("files")
}
