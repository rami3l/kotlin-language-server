...
internal class MavenClassPathResolver private constructor(private val pom: Path) :
    ...
    override val classpath: Set<ClassPathEntry>
        get() {
            ...
            return artifacts
                .mapNotNull {
                    findMavenArtifact(it, false)?.let { it1 -> ClassPathEntry(it1, null) }
                }
                .toSet()
        }
    ...
    override val classpathWithSources: Set<ClassPathEntry>
        get() {
            ...
            return artifacts
                .mapNotNull {
                    findMavenArtifact(it, false)?.let { it1 ->
                        ClassPathEntry(
                            it1,
                            if (it.source) findMavenArtifact(it, it.source) else null
                        )
                    }
                }
                .toSet()
...
private fun readMavenDependencyListWithSources(
    ...
    artifacts.forEach {
        it.source =
            sources.any { it1 ->
                it1.group == it.group && it1.artifact == it.artifact && it1.version == it.version
            }
    }
...
private fun findMavenArtifact(a: Artifact, source: Boolean): Path? {
    ...
    return if (Files.exists(result)) result
    else {
...
private fun mavenJarName(a: Artifact, source: Boolean) =
    if (source) "${a.artifact}-${a.version}-sources.jar" else "${a.artifact}-${a.version}.jar"
    ...
    return when (parts.size) {
        ...
        6 ->
            ...
            throw IllegalArgumentException(
                "$rawArtifact is not a properly formed Maven/Gradle artifact"
            )
    }
}
        ...
        5 ->
            if (parts[3] == "sources")
                Artifact(
                    ...
                    version = version
                            ?: parts[4].split(" ")[
                                0], // Needed to avoid the rest of the line from being captured.
                    scope = null,
                    source = true
                )
            else null
