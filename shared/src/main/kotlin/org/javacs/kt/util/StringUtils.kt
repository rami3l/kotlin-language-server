...
/**
 * Computes a string distance using a slightly modified variant of the SIFT4 algorithm in linear
 * time. Note that the function is asymmetric with respect to its two input strings and thus is not
 ...
 */
fun stringDistance(candidate: CharSequence, pattern: CharSequence, maxOffset: Int = 4): Int =
    when {
        ...
        pattern.length == 0 -> candidate.length
            ...
            while (iCandidate < candidateLength && iPattern < patternLength) {
                if (candidate[iCandidate] == pattern[iPattern]) {
                    ...
                    searchWindow@ for (i in 0 until maxOffset) {
                        when {
