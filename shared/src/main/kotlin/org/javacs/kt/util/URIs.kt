...
/**
 * Parse a possibly-percent-encoded URI string. Decoding is necessary since some language clients
 * (including VSCode) invalidly percent-encode colons.
 */
fun parseURI(uri: String): URI =
    URI.create(
        runCatching {
                URLDecoder.decode(uri, StandardCharsets.UTF_8.toString()).replace(" ", "%20")
            }
            .getOrDefault(uri)
    )

val URI.filePath: Path?
    get() = runCatching { Paths.get(this) }.getOrNull()
...
fun describeURIs(uris: Collection<URI>): String =
    ...
    else if (uris.size > 5) "${uris.size} files" else uris.map(::describeURI).joinToString(", ")
