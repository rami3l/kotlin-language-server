...
/**
 * Computes LSP-encoded semantic tokens for the given range in the document. No range means the
 * entire document.
 */
...
/**
 * Computes semantic tokens for the given range in the document. No range means the entire document.
 */
        ...
        if (token.range.start.line == token.range.end.line) {
            ...
            val deltaStart =
                token.range.start.character -
                    (last?.takeIf { deltaLine == 0 }?.range?.start?.character ?: 0)
...
private fun encodeModifiers(modifiers: Set<SemanticTokenModifier>): Int =
    modifiers.map { 1 shl it.ordinal }.fold(0, Int::or)
...
private fun elementTokens(
    ...
    return element
        // TODO: Ideally we would like to cut-off subtrees outside our range, but this doesn't quite
        // seem to work
...
private fun elementToken(element: PsiElement, bindingContext: BindingContext): SemanticToken? {
    ...
    return when (element) {
        ...
        is KtNameReferenceExpression -> {
            ...
            val tokenType =
                when (target) {
                    ...
                    is ConstructorDescriptor ->
                        when (target.constructedClass.kind) {
                            ClassKind.ENUM_ENTRY -> SemanticTokenType.ENUM_MEMBER
                            ClassKind.ANNOTATION_CLASS ->
                                SemanticTokenType.TYPE // annotations look nicer this way
            ...
            val isConstant =
                (target as? VariableDescriptor)?.let { !it.isVar() || it.isConst() } ?: false
        ...
        is PsiNameIdentifierOwner -> {
            ...
            val identifierRange =
                element.nameIdentifier?.let { range(file.text, it.textRange) } ?: return null
            val modifiers = mutableSetOf(SemanticTokenModifier.DECLARATION)

            if (element is KtVariableDeclaration &&
                    (!element.isVar() || element.hasModifier(KtTokens.CONST_KEYWORD)) ||
