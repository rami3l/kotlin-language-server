...
private fun doFindReferences(file: Path, cursor: Int, sp: SourcePath): Collection<KtElement> {
    ...
    val element =
        recover.elementAtPoint(cursor)?.findParent<KtNamedDeclaration>()
...
private fun doFindReferences(element: KtNamedDeclaration, sp: SourcePath): Collection<KtElement> {
    ...
    val declaration =
        recover.compile[BindingContext.DECLARATION_TO_DESCRIPTOR, element]
    ...
    return when {
        isComponent(declaration) ->
            findComponentReferences(element, recompile) + findNameReferences(element, recompile)
        isIterator(declaration) ->
            findIteratorReferences(element, recompile) + findNameReferences(element, recompile)
        isPropertyDelegate(declaration) ->
            findDelegateReferences(element, recompile) + findNameReferences(element, recompile)
...
private fun findDelegateReferences(
    ...
    return references.filter { matchesReference(it.value.candidateDescriptor, element) }.map {
        it.value.call.callElement
    }
}
...
private fun possibleReferences(declaration: DeclarationDescriptor, sp: SourcePath): Set<KtFile> {
    ...
    if (declaration is FunctionDescriptor &&
            ...
            declaration.name == OperatorNameConventions.INVOKE
    ) {
        return possibleInvokeReferences(declaration, sp) +
            possibleNameReferences(declaration.name, sp)
    }
    if (declaration is FunctionDescriptor) {
...
private fun hasForLoops(sp: SourcePath): Set<KtFile> = sp.all().filter(::hasForLoop).toSet()
...
private fun isGetSet(declaration: DeclarationDescriptor) =
    declaration is FunctionDescriptor &&
        declaration.isOperator &&
        (declaration.name == OperatorNameConventions.GET ||
            declaration.name == OperatorNameConventions.SET)

private fun possibleGetSets(sp: SourcePath): Set<KtFile> = sp.all().filter(::possibleGetSet).toSet()
...
private fun possibleInvokeReference(
    ...
    source: KtFile
): Boolean = source.preOrderTraversal().filterIsInstance<KtCallExpression>().any()
...
private fun possibleComponentReference(source: KtFile): Boolean =
    source.preOrderTraversal().filterIsInstance<KtDestructuringDeclarationEntry>().any()
...
private fun possibleTokenReference(find: List<KtSingleValueToken>, source: KtFile): Boolean =
    source.preOrderTraversal().filterIsInstance<KtOperationReferenceExpression>().any {
        it.operationSignTokenType in find
    }
...
private fun possibleNameReference(declaration: Name, source: KtFile): Boolean =
    source.preOrderTraversal().filterIsInstance<KtSimpleNameExpression>().any {
        it.getReferencedNameAsName() == declaration
    }
...
private fun matchesReference(found: DeclarationDescriptor, search: KtNamedDeclaration): Boolean {
    ...
    else return found.findPsi() == search
}
