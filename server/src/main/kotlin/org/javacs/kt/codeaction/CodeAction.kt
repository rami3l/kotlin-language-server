...
val QUICK_FIXES = listOf(ImplementAbstractFunctionsQuickFix())
        ...
        listOf(
            Either.forLeft<Command, CodeAction>(
                Command(
                    ...
                    listOf(file.parse.toPath().toUri().toString(), range)
                )
            )
        )
    } else {
...
fun getQuickFixes(
    ...
    return QUICK_FIXES.mapNotNull { it.compute(file, range, diagnostics) }
}
