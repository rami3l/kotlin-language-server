...
class SourcePath(
    ...
    private inner class SourceFile(
        ...
        val kind: CompilationKind =
            if (path?.fileName?.toString()?.endsWith(".gradle.kts") ?: false)
    ...
    fun compileFiles(all: Collection<URI>): BindingContext {
        ...
        val (changedBuildScripts, changedSources) =
            allChanged.partition { it.kind == CompilationKind.BUILD_SCRIPT }
        ...
        val buildScriptsContext =
            compileAndUpdate(changedBuildScripts, CompilationKind.BUILD_SCRIPT)
        ...
        val combined =
            listOf(buildScriptsContext, sourcesContext).filterNotNull() +
                same.map { it.compiledContext!! }
    ...
    fun compileAllFiles() {
        ...
        // TopDownAnalyzer
        files.keys.forEach { compileFiles(listOf(it)) }
    }
     ...
     * were changed.
     */
    private fun refreshDependencyIndexes(container: ComponentProvider) =
        indexAsync.execute {
            if (indexEnabled) {
        ...
        files
            .flatMap { file ->
                ...
                if (compiledFile != null && compiledContainer != null) {
                    ...
                    module.getPackage(compiledFile.packageFqName)
                        .memberScope
    ...
    fun all(includeHidden: Boolean = false): Collection<KtFile> =
        files.values.filter { includeHidden || !it.isTemporary }.map {
            it.apply { parseIfChanged() }.parsed!!
        }
}
