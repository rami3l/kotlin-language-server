...
class CompiledFile(
    ...
    fun typeAtPoint(cursor: Int): KotlinType? {
        var cursorExpr =
            parseAtPoint(cursor, asReference = true)?.findParent<KtExpression>()
    ...
    fun referenceAtPoint(cursor: Int): Pair<KtExpression, DeclarationDescriptor>? {
        ...
        var cursorExpr =
            element?.findParent<KtExpression>()
                ?: return nullResult(
                    "Couldn't find expression at ${describePosition(cursor)} (only found $element)"
                )
     ...
     *
     * If the `asReference` flag is set, the method will attempt to convert a declaration (e.g. of a
     * class or a function) to a referencing expression before parsing it.
     */
    fun parseAtPoint(cursor: Int, asReference: Boolean = false): KtElement? {
        ...
        val psi =
            parse.findElementAt(oldCursor)
                ?: return nullResult("Couldn't find anything at ${describePosition(cursor)}")
        val oldParent =
            psi.parentsWithSelf.filterIsInstance<KtDeclaration>().firstOrNull {
                it.textRange.contains(oldChanged)
            }
                ?: parse
        ...
        val recompile =
            classPath.compiler.createKtFile(
    ...
    /**
     * Extracts the surrounding content and the text offset from a PSI element.
     *
    ...
    private fun contentAndOffsetFromElement(
        ...
        if (asReference) {
            ...
            when {
                parent is KtClass && psi.node.elementType == KtTokens.IDENTIFIER -> {
                    // Converting class name identifier: Use a fake property with the class name as
                    // type
                    //                                   Otherwise the compiler/analyzer would throw
                    // an
                    // exception due to a missing TopLevelDescriptorProvider
                    val prefix = "val x: "
        ...
        surroundingContent =
            content.substring(
                recoveryRange.startOffset,
                content.length - (parse.text.length - recoveryRange.endOffset)
            )
    ...
    /**
     * Get the typed, compiled element at `cursor`. This may be out-of-date if the user is typing
     * quickly.
     */
    ...
    /**
     * Find the lexical-scope surrounding `cursor`. This may be out-of-date if the user is typing
     * quickly.
     */
    fun scopeAtPoint(cursor: Int): LexicalScope? {
        ...
        return compile
            ...
            .asSequence()
            .filter {
                it.key.textRange.startOffset <= oldCursor && oldCursor <= it.key.textRange.endOffset
            }
            .sortedBy { it.key.textRange.length }
            .map { it.value }
            .firstOrNull()
