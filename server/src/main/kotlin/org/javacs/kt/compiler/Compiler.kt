...
/** Kotlin compiler APIs used to parse, analyze and compile files and expressions. */
private class CompilationEnvironment(javaSourcePath: Set<Path>, classPath: Set<Path>) : Closeable {
    ...
    init {
        environment =
            KotlinCoreEnvironment.createForProduction(
                ...
                // Not to be confused with the CompilerConfiguration in the language server
                // Configuration
                configuration =
                    KotlinCompilerConfiguration().apply {
                        ...
                        if (classPath.any {
                                GRADLE_DSL_DEPENDENCY_PATTERN.matches(it.fileName.toString())
                            }
                        ) {
                            ...
                            try {
                                ...
                                val scriptHostConfig =
                                    ScriptingHostConfiguration(
                                        defaultJvmScriptingHostConfiguration
                                    ) { configurationDependencies(JvmDependency(fileClassPath)) }
                                // TODO: KotlinScriptDefinition will soon be deprecated, use
                                //       ScriptDefinition.compilationConfiguration and its
                                // defaultImports
                                // instead
                                //       of KotlinScriptDefinition.dependencyResolver
                                // TODO: Use ScriptDefinition.FromLegacyTemplate directly if
                                // possible
                                // scriptDefinitions = scriptTemplates.map {
                                // ScriptDefinition.FromLegacyTemplate(scriptHostConfig,
                                // scriptClassLoader.loadClass(it).kotlin) }
                                scriptDefinitions.addAll(
                                    scriptTemplates.map {
                                        ScriptDefinition.FromLegacy(
                                            ...
                                            object :
                                                ...
                                                override val dependencyResolver:
                        ...
                        LOG.info(
                            "Adding script definitions ${scriptDefinitions.map { it.asLegacyOrNull<KotlinScriptDefinition>()?.template?.simpleName }}"
                        )
    ...
    fun updateConfiguration(config: CompilerConfiguration) {
        jvmTargetFrom(config.jvm.target)?.let {
            environment.configuration.put(JVMConfigurationKeys.JVM_TARGET, it)
        }
    }

    private fun jvmTargetFrom(target: String): JvmTarget? =
        when (target) {
            // See
    ...
    ): Pair<ComponentProvider, BindingTraceContext> {
...
/** Determines the compilation environment used by the compiler (and thus the class path). */
...
/**
 * Incrementally compiles files and expressions. The basic strategy for compiling one file at-a-time
 * is outlined in OneFilePerformance.
 */
class Compiler(
    ...
    private val buildScriptCompileEnvironment =
        buildScriptClassPath.takeIf { it.isNotEmpty() }?.let {
            CompilationEnvironment(emptySet(), it)
        }
    ...
    /**
     * Updates the compiler environment using the given configuration (which is a class from this
     * project).
     */
    fun updateConfiguration(config: CompilerConfiguration) {
    ...
    fun createKtDeclaration(
        ...
        if (onlyDeclaration is KtScript) {
            ...
            assert(declarations.size == 1) {
                "${declarations.size} declarations in script in $content"
            }
    ...
    private fun compileEnvironmentFor(kind: CompilationKind): CompilationEnvironment =
        when (kind) {
            CompilationKind.DEFAULT -> defaultCompileEnvironment
            CompilationKind.BUILD_SCRIPT -> buildScriptCompileEnvironment
    ...
    fun compileKtFile(
        ...
        sourcePath: Collection<KtFile>,
        kind: CompilationKind = CompilationKind.DEFAULT
    ): Pair<BindingContext, ComponentProvider> = compileKtFiles(listOf(file), sourcePath, kind)
    ...
    fun compileKtFiles(
        ...
        if (kind == CompilationKind.BUILD_SCRIPT) {
            ...
            files.forEach {
                LOG.debug {
                    "$it -> ScriptDefinition: ${it.findScriptDefinition()?.asLegacyOrNull<KotlinScriptDefinition>()?.template?.simpleName}"
                }
            }
        }
    ...
    fun compileKtExpression(
        ...
        try {
            ...
            throw KotlinLSException(
                "Error while analyzing: ${describeExpression(expression.text)}",
                e
            )
        }
    }
...
private fun describeExpression(expression: String): String =
    expression.lines().let { lines ->
        if (lines.size < 5) {
            expression
        } else {
