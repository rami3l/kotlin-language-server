...
private fun typeHoverAt(file: CompiledFile, cursor: Int): Hover? {
    ...
    val hoverText =
        renderTypeOf(expression, file.bindingContextOf(expression, scope)) ?: return null
    val hover =
        MarkupContent(
            "markdown",
            listOf("```kotlin\n$hoverText\n```", javaDoc)
...
// Source:
// https://github.com/JetBrains/kotlin/blob/master/idea/src/org/jetbrains/kotlin/idea/codeInsight/KotlinExpressionTypeProvider.kt
...
private fun renderJavaDoc(text: String): String {
    ...
    return split
        .mapIndexed { i, it ->
            var ret: String
            if (i == 0)
                ret = it.substring(it.indexOf("/**") + 3) // get rid of the start comment characters
            else if (i == split.size - 1)
                ret = it.substring(it.indexOf("*/") + 2) // get rid of the end comment characters
...
private fun renderTypeOf(element: KtExpression, bindingContext: BindingContext): String? {
        ...
        if (descriptor != null) {
            when (descriptor) {
                is CallableDescriptor ->
                    return descriptor.returnType?.let(TYPE_RENDERER::renderType)
            }
        }
    }

    val expressionType =
        bindingContext[BindingContext.EXPRESSION_TYPE_INFO, element]?.type
            ?: element.getType(bindingContext)
    ...
    if (smartCast != null && element is KtReferenceExpression) {
        val declaredType =
            (bindingContext[BindingContext.REFERENCE_TARGET, element] as? CallableDescriptor)
                ?.returnType
