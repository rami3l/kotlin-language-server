...
class LintTest : SingleFileTestFixture("lint", "LintErrors.kt") {
    ...
    @Test
    fun `only lint once for many edits in a short period`() {
        var text = "1"
        for (i in 1..10) {
            val newText = text + "1"
    ...
    @Test
    fun `linting should not be dropped if another linting is running`() {
        var callbackCount = 0
        languageServer.textDocumentService.debounceLint.waitForPendingTask()
        languageServer.textDocumentService.lintRecompilationCallback =
            {
                if (callbackCount++ == 0) {
                    ...
                    languageServer
                        .textDocumentService
                        .documentSymbol(
                            DocumentSymbolParams(TextDocumentIdentifier(uri(file).toString()))
                        )
                        .get()
                }
            }
